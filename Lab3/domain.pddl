(define (domain ghostandrobot)
    ...
    (:types
        ...
        robot ghost - entity
        timestep - object
    ) ; Two gridindex identify a position in the board.; Robot and ghost are the entities that move through positions; timestep is an identificator for the sequence of movements of the ghost  
    (:predicates 
        (end); This predicate becomes true through an action that checks ghost and player are in the same position.
        (just_less ?i - gridindex ?j - gridindex) ; Example: just_less 1 2. just_less 1 4 is false.
        (at ?e - entity ?i - gridindex ?j - gridindex) ; Entity e is at coordinates i, j; The two following predicates tell whether I can move the entities BEFORE advancing the time.
        (movedghost)
    ...
    (:functions
        (time)
        (cost)
        (positioncost ?i - gridindex ?j - gridindex) ; Moving or staying in position (i, j)'s cost.
    )
    (:action advancetime 
        :parameters () 
